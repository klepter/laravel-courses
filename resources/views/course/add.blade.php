    ...
    <div class="register">
        ...
        <form action="{{route('course.add')}}" method="POST" enctype="multipart/form-data">
            ...
            <div>
                ...
                <input id="amount" type="number" name="amount" min="1" value="{{old('amount')}}"
                       required/>
...
{{--                       min="{{str_replace(' ', 'T', \Carbon\Carbon::now()->format('Y-m-d H:i'))}}"--}} name="start_date"
                ...
                <select name="code_id" id="code_id">
                    @foreach($languages as $language)
                        <option value="{{$language->id}}"
                                @if(old('code_id') == $language->id) selected @endif>
                            {{$language->title}}
                        </option>
                    @endforeach
                </select>
